...
#: Valeurs de cuisine conservées par défaut (indépendants / formats ciblés).
...
def parse_cuisine_values(raw: str | None) -> tuple[str, ...]:
    """Parse une liste CSV de cuisines (`pizza,cafe`) en tuple normalisé.

    Si `raw` est vide ou None, retourne `DEFAULT_CUISINES`.
    """
    ...
    return cleaned or DEFAULT_CUISINES
...
class CuisineFilter:
    ...
    def __init__(self, allowed: tuple[str, ...] | None = None) -> None:
        raw = allowed if allowed is not None else DEFAULT_CUISINES
        self._allowed = {normalize_text(value).replace(" ", "_") for value in raw if value}
